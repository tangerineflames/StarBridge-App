...
def ack_alert(aid: int, db: Session = Depends(get_db)):
    o = db.get(Alert, aid)  # 推荐用 Session.get
    if not o:
...
# 视频：UDP 接收（后台线程） + MJPEG 输出

# 配置
UDP_IP = "0.0.0.0"   # 监听所有网卡
UDP_PORT = 8080      # 与发送端一致
UDP_RECV_BUFSIZE = 1024 * 1024  # 1MB
FRAME_SIZE = (360, 640)         # 占位图尺寸
...
def _encode_jpeg(img: np.ndarray) -> Optional[bytes]:
    ok, buf = cv2.imencode(".jpg", img)  # 可按需设置 [int(cv2.IMWRITE_JPEG_QUALITY), 80]
    if not ok:
        return None
    return buf.tobytes()
...
def _on_shutdown():
    global _stop_flag
    _stop_flag = True
    # 线程是 daemon，正常会随进程退出；这里设置了超时退出机制
